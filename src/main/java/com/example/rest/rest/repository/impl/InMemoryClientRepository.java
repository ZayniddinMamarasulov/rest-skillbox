...
public class InMemoryClientRepository implements ClientRepository {
    ...
    public Client update(Client client) {
        ...
        Client existingClient = repository.get(clientId);
        if (existingClient == null) {
            throw new EntityNotFoundException(MessageFormat.format("Client with id %d not found", clientId));
        }

        BeanUtils.copyNonNullProperties(client, existingClient);
        existingClient.setId(clientId);

        repository.put(clientId, existingClient);
    ...
    public void deleteById(Long id) {
        ...
        if (client == null) {
            throw new EntityNotFoundException(MessageFormat.format("Client with id %d not found", id));
        }
